Page({
  ...
  async getList() {
    return wx.cloud.callFunction({ name: 'getList', data: { list: getApp().globalData.collectionMarketList } }).then(data => {
  ...
  async slideButtonTap(element, isUpper) {
    ...
    await wx.cloud.callFunction({ name: 'getOpenId' }).then(async openid => {
      ...
      if (index === 0) {
        ...
        //处理星标按钮点击事件
        if (index === 1) {
          await wx.cloud.callFunction({ name: 'editStar', data: { _id: item._id, list: getApp().globalData.collectionMarketList, value: !item.star } })
          //更新本地数据
          item.star = !item.star
        }
        ...
        //处理删除按钮点击事件
          ...
          await wx.cloud.callFunction({ name: 'deleteElement', data: { _id: item._id, list: getApp().globalData.collectionMarketList } })
        }
